...
#
# Modified 
# Includes the possibility to set the initial state
# and to print what content is sent to the function (update)
...
class sha256(object):
    ...
    def __init__(self, m=None, s=None, l=0):        
        self._buffer = b''

        # Define original state
        if s is not None:
            print("  SHA256: Setting start state")
            self._h = s
            self._counter = l
        else:
            print("  SHA256: Using original start state")
    ...
    def update(self, m):
        print("  SHA256: Update: ", binascii.hexlify(m).decode('latin'))
